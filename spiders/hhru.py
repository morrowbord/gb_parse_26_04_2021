...
class HhruSpider(scrapy.Spider):
    ...
    def vacansy_parse(self, response: HtmlResponse):
        ...
        url = response.url
        # print(name, salary)
        # ссылка на компанию в теге альт, не понял как организовать переход по ссылке(follow) и выход обратно(collback)
        # url с описанием компании по другому урл ниже:
        # company = response.css('.employer-sidebar__logo-img-container .employer-sidebar__logo::attr(alt)').get()
        yield JobparserItem(name=name, url=url, salary=salary, company=company, opisanie=opisanie, key_skills=key_skills)
